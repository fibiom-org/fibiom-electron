...
const loadWhisper = (onProgress: (pct: number | null) => void): Promise<string> => {
...
export function registerSpeechHandlers(): void {
  ipcMain.handle('speech:transcribe', async (event, pcm: Uint8Array) => {
    const id = await loadWhisper((pct) => event.sender.send('speech:progress', pct))
    const text = await transcribe({ modelId: id, audioChunk: Buffer.from(pcm) })
  ...
  ipcMain.handle('speech:speak', async (event, text: string) => {
    ...
    const id = await loadTts((pct) => event.sender.send('speech:progress', pct))
    const result = textToSpeech({ modelId: id, text: trimmed, inputType: 'text', stream: false })
    const samples = await result.buffer
    console.log(`[speech] synthesized ${samples.length} samples for ${trimmed.length} chars`)
    return toWav(samples, TTS_SAMPLE_RATE)
  })

  ipcMain.handle('speech:unload', async () => {
    await Promise.all([
      whisperId ? unloadModel({ modelId: whisperId, clearStorage: false }) : Promise.resolve(),
      ttsId ? unloadModel({ modelId: ttsId, clearStorage: false }) : Promise.resolve()
    ])
    whisperId = null
    ttsId = null
  })
