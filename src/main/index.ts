...
function registerAuthHandlers(): void {
  ...
  ipcMain.handle("auth:setup", (_event, masterKey: string) =>
    secureStore.setup(masterKey),
  );
  ipcMain.handle("auth:unlock", (_event, masterKey: string) =>
    secureStore.unlock(masterKey),
  );
