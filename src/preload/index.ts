  ...
  try {
    ...
    contextBridge.exposeInMainWorld("authAPI", {
      ...
      setup: (masterKey: string): Promise<void> =>
        ipcRenderer.invoke("auth:setup", masterKey),
      unlock: (masterKey: string): Promise<boolean> =>
        ipcRenderer.invoke("auth:unlock", masterKey),
    ...
    contextBridge.exposeInMainWorld("visionAPI", {
      ...
      onProgress: (cb: (percentage: number | null) => void): (() => void) => {
        const listener = (_event: unknown, percentage: number | null): void =>
          cb(percentage);
    ...
    contextBridge.exposeInMainWorld("chatAPI", {
      ...
      generateTitle: (chatId: number) =>
        ipcRenderer.invoke("chat:generateTitle", chatId),
    });
  } catch (error) {
    console.error(error);
  }
